...
class ToneSlapperWindow(QMainWindow):
    ...
    def __init__(self) -> None:
        ...
        self._signals.scan_device.connect(self._scan_device_discovered)
        self._signals.scan_completed.connect(self._scan_complete)
        self._signals.scan_failed.connect(self._scan_failed)
        ...
        sections = (
            ...
            (
                ...
                (
                    ...
                    "copy from the Tone Slapper GitHub repository.",
                ),
            ),
            (
    ...
    def _add_scanned_device(self, device) -> bool:
        ...
        for display, existing_address in self.devices.items():
            if existing_address.casefold() == address.casefold():
                self.devices[display] = address
                self.device_models[display] = model
                return False

        display = model or profile.display_name
        duplicate_number = 2
        while display in self.devices:
            display = f"{model or profile.display_name} ({duplicate_number})"
            duplicate_number += 1
        self.devices[display] = address
        self.device_models[display] = model
        self.device_combo.blockSignals(True)
    ...
    def _offer_github_oem(self, detail: str) -> None:
        choice = self._ask_oem_action(
            "OEM download failed",
            f"{detail}\n\n"
            "Download the pinned OEM recovery file from the Tone Slapper "
    ...
    def _show_manual_oem_recovery(self, detail: str) -> None:
        ...
        content.addWidget(
            self._help_label(
                f"{detail}\n\n"
                "Download the OEM file manually from the Tone Slapper GitHub "
