...
async def scan_devices(
    *,
    ...
    def record_live(device: Any, advertisement: Any) -> None:
        ...
        key = result.address.casefold()
        is_new = key not in results
        results[key] = result
        if is_new and on_discovered is not None:
    ...
    if on_discovered is None:
        ...
        for device, advertisement in discovered.values():
            record_live(device, advertisement)
    else:
        live_scanner = scanner(
            detection_callback=record_live,
            scanning_mode="active",
        )
        ...
        for device, advertisement in (
            live_scanner.discovered_devices_and_advertisement_data.values()
        ):
            record_live(device, advertisement)
    ...
    for cached in cached_provider():
        ...
        if _device_identity(cached.name) in live_identities:
            continue
        key = cached.address.casefold()
        if key in results:
            continue
        results[key] = cached
        if on_discovered is not None:
            on_discovered(cached)
    return sorted(
        results.values(),
        key=lambda device: (
            device.remembered,
